...
function cf_update_or_create_pipedrive_person($data, $api_key, $api_url, $post_id, $person_id = null)
{
  ...
  update_post_meta($post_id, 'debug_person', json_encode($result, JSON_PRETTY_PRINT));

  if (isset($result['success']) && !$result['success'] && isset($result['error']) && strpos($result['error'], 'Invalid field(s)') !== false) {
    cf_send_error_email($post_id, $result['error']);
    return false;
  }
...
function cf_send_error_email($post_id, $error_message)
{
  $admin_url = "https://sjednej.cz/wp-admin/post.php?post={$post_id}&action=edit";
  $to = ['jsme@webotvurci.cz', 'l.kubaskova@sjednej.cz'];
  $subject = "Chyba při zakládání osoby v Pipedrive - ID {$post_id}";
  $message = "Došlo k chybě při vytváření osoby v Pipedrive.\n\n";
  $message .= "Chyba: {$error_message}\n";
  $message .= "Odkaz do administrace: {$admin_url}\n";
